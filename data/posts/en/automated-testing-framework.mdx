...
4. The client polls a `GET` endpoint to observe state transitions (`QUEUED` $\rightarrow$ `RUNNING` $\rightarrow$ `SUCCESS` | `FAILED` | `SKIPPED`).
...
> **Note on CI Visuals:** By mapping the internal `SKIPPED` status to JUnit's `Assumptions.assumeTrue(false)`, native test runners register the execution as *Aborted*. CI/CD tools like Jenkins consume this report and properly classify the test as unstable or skipped, preventing false-positive pipeline failures.
...
If this evaluation fails, the REST API returns a `SKIPPED` payload. In CI, JUnit reports an aborted test. This reduces the compute time of an invalid test from 20 seconds to 50 milliseconds.
...
By treating an E2E test as a parameterized domain model (`TestCase` $\rightarrow$ `TestRunner`), the architecture provides significant flexibility. The same Java code that runs synchronously in a nightly Jenkins build powers an asynchronous, interactive frontend dashboard, complete with type safety at the Java boundary and smart precondition evaluation.
