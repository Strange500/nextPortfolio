...
4. Le client interroge périodiquement (polling) un point de terminaison `GET` pour observer les transitions d'état (`QUEUED` $\rightarrow$ `RUNNING` $\rightarrow$ `SUCCESS` | `FAILED` | `SKIPPED`).
...
Si cette évaluation échoue, l'API REST renvoie une charge utile `SKIPPED`. Dans la CI, JUnit signale un test avorté. Cela réduit le temps de calcul d'un test invalide de 20 secondes à 50 millisecondes.
